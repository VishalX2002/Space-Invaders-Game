(() => {
  ...
  class EnemyBullet {
    ...
    update(dt) {
      this.y += this.speed * dt;
      if (this.y > WORLD.h + 20) this.alive = false;
    }
    draw() {
  ...
  function update(dt) {
    ...
    if (enemies.length) {
      if (minX <= 10 && swarm.direction < 0) {
        swarm.direction = 1;
        dropSwarm();
      }
      if (maxX >= WORLD.w - 10 && swarm.direction > 0) {
        swarm.direction = -1;
        dropSwarm();
      }
    }
    enemies.forEach((e) => {
      e.x += swarm.dx * dt * swarm.direction;
    });
    ...
    if (swarm.timer <= 0 && enemies.length) {
      ...
      enemyBullets.push(
        new EnemyBullet(shooter.x + shooter.w / 2, shooter.y + shooter.h)
      );
    }
  ...
  function draw() {
    ...
    flashes.forEach((f) => {
      ...
      ctx.fillStyle = f.c;
      ctx.beginPath();
      ctx.arc(f.x, f.y, 16 * (1 + (1 - f.t) * 3), 0, Math.PI * 2);
      ctx.fill();
      ctx.globalAlpha = 1;
    });
  ...
  let flashes = [];
  function flash(x, y, c) {
    flashes.push({ x, y, c, t: 0.5 });
  }
  ...
  function gameOver() {
    ...
    ui.menu.querySelector("#how").addEventListener("click", () => {
      ui.howto.style.display = "grid";
    });
  }
  ...
  ui.howClose.addEventListener("click", () => {
    ui.howto.style.display = "none";
  });
